...
export function CartProvider({ children }: CartProviderProps): JSX.Element {
  ...
  const addProduct = async (productId: number) => {
    try {
      const productOnTheCart = cart.find(product => product.id === productId);
      ...
      if(productOnTheCart) {
        ...
        if(stock.amount > productOnTheCart.amount) {
  ...
  const updateProductAmount = async ({
    ...
    try {
      ...
      const notAvaliableStock = amount > productAmount

      if(notAvaliableStock) {
        toast.error('Quantidade solicitada fora de estoque')
